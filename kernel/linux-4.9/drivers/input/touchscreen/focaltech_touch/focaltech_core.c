...
/*****************************************************************************
*  Name: sunxi_pin_config
*  Brief:
*  Input:
*  Output:
*  Return:
*****************************************************************************/
static int sunxi_pin_config(struct gpio_config pin_cfg)
{
	int ret = 0;
	char pin_name[32];
	u32 config;
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	/* reset, irq gpio info */
	pdata->reset_gpio = of_get_named_gpio_flags(np, "focaltech,reset-gpio", 0, (enum of_gpio_flags *)&pin_cfg);
	if (pdata->reset_gpio < 0) {
		FTS_ERROR("Unable to get reset_gpio");
	}
	sunxi_pin_config(pin_cfg);

	pdata->irq_gpio_flags = 0;
	pdata->irq_gpio = of_get_named_gpio_flags(np, "focaltech,irq-gpio", 0, (enum of_gpio_flags *)&pin_cfg);
